...
2b. **Funil de propostas — evoluções** (entregue na Sessão 51, `/shows/funil` + `showPipeline`,
   ...
   **Nudge no Painel** entregue na Sessão 247 — `staleProposalsHeadline(report)` + tipo `StaleProposalsHeadline` em
   ...
   propostas decididas, ano anterior do mesmo acervo já carregado, zero I/O extra; +4 testes); (b) um nudge no
   ...
   nela (o CSV já foi entregue na Sessão 243/D237). A **coluna "vs. {ano-1}" no CSV do funil por contratante** foi
   ...
   dos mesmos `items` já carregados, zero I/O extra; +5 testes). Próximo possível para esta feature: (a) coluna "vs. {ano-1}" por
   linha na tabela (no molde de `indexContactPipelineChanges`/D238, o polimento barato que o card de movers abre); (b) nudge no
   Painel de contratante cuja conversão despencou (D247 alt. (e) / D248 alt. (b)) — adiável, o Painel já tem o nudge da conversão
   agregada (`proposalConversionHeadline`/D245) e um 2º eixo por contratante pode ficar barulhento.
