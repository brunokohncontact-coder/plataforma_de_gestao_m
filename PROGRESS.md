...
AGREGADO de cada grupo (`totalNet`) — um local/cidade lucrativo no total pode esconder shows individuais no prejuízo (a informação acionável
"em que casa/praça eu às vezes toco no vermelho, e quanto R$ isso custou"). **(1)** camada pura em `src/lib/finance.ts`: novos campos
`lossCount` (nº de shows do grupo com `net < 0`) e `lossNet` (prejuízo somado desses shows, centavos ≤0; `0` quando nenhum) em `VenueProfitRow`
(e portanto em `CityProfitRow`, que é `= VenueProfitRow`). Acumulados no MESMO `if (pnl.net < 0)` dentro do agregador COMPARTILHADO
`aggregateShowProfit` — a fonte única de `rankVenuesByProfit` E `rankCitiesByProfit` (a cidade é rollup natural das casas nela) — reusando o
`computeShowPnL` já iterado; zero consulta/assinatura nova, uma edição cobre ambos os eixos. Mesma faixa "loss" de `showResultDistribution`
(o zerado, `net === 0`, NÃO conta) e mesma convenção `lossNet ≤ 0` das D377/D379, para os números baterem entre superfícies. **(2)** colunas
"No vermelho" e "Prejuízo" nas DUAS telas (`shows/locais/page.tsx` e `shows/cidades/page.tsx`), logo à direita de "Shows" (o par quantos↔quanto
lado a lado): `lossCount`/`formatMoney(lossNet)` em vermelho/negrito quando `lossCount > 0` (com `title` explicativo), "—" cinza quando 0; nota
de rodapé atualizada nas duas. **(3)** colunas "No vermelho" e "Prejuízo (R$)" no serializador CSV COMPARTILHADO `venueProfitToCsv`
(`src/lib/csv.ts`), na MESMA posição (entre "Shows" e "Cachê (R$)") — `String(row.lossCount)` e `centsToCsvAmount(row.lossNet)` (mantém o
sinal; "0,00" quando nenhum, CSV orientado a máquina) — cobrindo os DOIS exports (`/shows/locais/export` e `/shows/cidades/export`) de uma vez.
**+3 testes** de CSV (`csv.test.ts`: header com as duas colunas; linha com `lossCount`/`lossNet` populados e sinal negativo; índices de "cachê
mediano" deslocados +2) **+2 testes** de lógica (`finance.test.ts`: por local — 1 de 3 shows no vermelho, agregado ainda positivo, o empate
NÃO conta, `lossNet=-50_00` só do vermelho + zerado sem vermelho; por cidade — rollup soma o vermelho de casas distintas da mesma praça, Olinda
sem vermelho). Camada pura (agregador compartilhado) + wiring; zero migração/dependência. DoD verde: `npm run build` (as 4 superfícies
compilam; exports seguem 0 B), `npx tsc --noEmit`, `npm run lint` (0 warnings), `npm test` (**2103 testes**); smoke → `/login` 200,
`/shows/locais?ano=2025`, `/shows/cidades?ano=2025` e os dois `/export?ano=2025` 307→/login (auth-gated); `npm audit` inalterado (10 advisories:
4 moderate/5 high/1 critical, ZERO dependência nova), ver D380. **Próximo possível** — (a) surtir o mesmo par count↔R$ no eixo por PAPEL do
contratante já existe (D377/D379); estender o vermelho ao comparativo ano a ano por cidade/local (`cityProfitComparisonToCsv`, uma coluna Δ
vermelho) fecharia o trio tela↔export↔comparativo desse eixo geográfico; (b) levar o recorte por natureza (todos × só firmes) ao eixo de papel
(tela + card + export em conjunto, a consistência futura da D374); (c) ecoar o "quanto R$ no vermelho nesta casa/praça" como nudge geográfico
no Painel. Fora deste eixo, segue como único pendente do backup a restauração por MERGE (ALTO risco, revisão humana).
**Antes disso, Sessão 384 — COLUNA "PREJUÍZO" (`lossNet`) EM `ContactProfitRow`/`RoleProfitRow`: O "QUANTO" R$ AO LADO DO "QUANTOS" (`lossCount`) — TELA + EXPORT (D379):**
...
`roleMarginComparisonToCsv(comparison)` em `src/lib/csv.ts`: planilha orientada a ITEM (molde de `contactMarginComparisonToCsv`/D373),
uma linha por PAPEL presente nos DOIS anos (o grupo "sem contratante" já é ignorado pela camada pura), preservando a ordem da comparação
(maior aperto primeiro); colunas Papel, Margem ano anterior/corrente (%), Δ margem (p.p.), Resultado ano anterior/corrente (R$), Δ
resultado (R$), Shows ano anterior/corrente, e "Situação" (Apertou a margem / Ganhou margem / Estável) classificada com o mesmo
`ROLE_MARGIN_DROP_EPSILON`=0,05. Sem linha "Total" (margem é razão, não soma). Rótulo do papel via `contactRoleLabel` já existente.
**(2)** rota `src/app/(app)/contatos/rentabilidade/por-papel/comparativo-margem/export/route.ts`: espelha o gate do card e o irmão por
contratante — exige `?ano=YYYY` concreto (senão 404) E ≥1 papel em comum (`comparedCount===0` → 404); reusa
...
**+4 testes** de CSV (`csv.test.ts`: layout item + Δ assinado + maior aperto primeiro; "Estável" no limiar; agrega por papel — duas casas
do mesmo papel numa linha só; só cabeçalho sem papel em comum). Camada pura + wiring; zero migração/dependência. DoD verde: `npm run build`
(rota nova registrada, 0 B), `npx tsc --noEmit`, `npm run lint` (0 warnings), `npm test` (**2092 testes**); smoke → `/login` 200, a rota
nova e a página 307→/login (auth-gated); **smoke autenticado diferencial** (usuário smoke; PROMOTER margem 100%→50% por despesa, BOOKER
60%→80%, VENUE só em 2025) → `?ano=2025` **200** `text/csv` (filename `…-2025-vs-2024.csv`) com linhas
"Produtor/Promoter;100%;50%;-50;100,00;50,00;-50,00;1;1;Apertou a margem" e "Contratante;60%;80%;+20;…;Ganhou margem", **VENUE AUSENTE**
(só voltou quem contratou nos dois anos); sem `?ano` **404** e `?ano=2024` (2023 vazio) **404** (gate preservado); `npm audit` inalterado
(10 advisories: 4 moderate/5 high/1 critical, ZERO dependência nova), ver D376. **Próximo possível** — (a) adicionar a CONTAGEM de shows
no vermelho por papel/contratante (o outro lado do par contagem↔margem, exigiria novos campos em `RoleProfitRow`/`ContactProfitRow`);
(b) levar o recorte por natureza (todos × só firmes) ao eixo de papel (tela + card + este export em conjunto, a consistência futura da
D374). O limiar `ROLE_MARGIN_DROP_EPSILON`(D375) segue **hipótese** (ver Bloqueios). Fora deste eixo, segue como único pendente do backup
a restauração por MERGE (ALTO risco, revisão humana).
**Antes disso, Sessão 380 — COMPARATIVO DE MARGEM POR PAPEL DO CONTRATANTE ANO A ANO: "que tipo de canal aperta a margem" (`compareRoleMargins`, D375):**
...
Então esta sessão entrega primeiro a leitura + validação, SEM gravar nada. Módulo puro `src/lib/accountImport.ts`:
`parseAccountDataExport(raw: unknown)` valida defensivamente um valor JÁ parseado (entrada NÃO CONFIÁVEL) contra o formato
do export e devolve `{ ok:true, data, summary, warnings }` ou `{ ok:false, errors }` (nunca lança); `parseAccountDataExportJson(text)`
embrulha o `JSON.parse` (sintaxe inválida → erro amigável). ERROS bloqueiam (não-objeto, `meta` ausente, app ≠ `Palco`,
`schemaVersion` fora de `SUPPORTED_ACCOUNT_IMPORT_SCHEMA_VERSIONS`=[1], lista de topo faltando, registro com campo
obrigatório mal-tipado, perfil sem nome/e-mail); AVISOS não bloqueiam (integridade referencial — `contactIds`/`showId`
órfãos —, ids duplicados, anos de meta repetidos, `meta.counts` que não bate). Opcionais ausentes → `null` (mesmo contrato
do export). Camada PURA (sem Prisma/`new Date()`/I/O), testável. Tela `/conta/dados/importar` (server component auth-gated)
com `<input type="file">` que posta na server action `previewAccountImportAction` (lê `file.text()`, teto de 8 MB, delega ao
parser, devolve via `useFormState`): mostra counts por entidade, `exportedAt` formatado, avisos em âmbar, erros em vermelho,
sempre reforçando "nada foi gravado". Link "🔍 Conferir um backup" na seção "Seus dados" de `/conta`. **+17 testes**
(`accountImport.test.ts`: round-trip do export e do JSON serializado, rejeições estruturais, avisos referenciais/duplicados/
counts, conta vazia, normalização opcionais→null, JSON malformado). DoD verde: `npm run build`, `npx tsc --noEmit`,
`npm run lint` (0 warnings), `npm test` (**1960 testes**); smoke → `/login` 200, `/conta` e `/conta/dados/importar`
307→/login (auth-gated); **smoke autenticado** (cookie do usuário demo) → `/conta/dados/importar` 200 com o formulário,
`/conta` com os dois botões, e o EXPORT REAL da conta demo (3 shows / 5 transações / 3 contatos) passando por
`parseAccountDataExportJson` → `ok`, `Palco` v1, counts batendo, 0 avisos (o exato fluxo da action); `npm audit` inalterado
(10 advisories: 4 moderate/5 high/1 critical, ZERO dependência nova), ver D355. **Próximo possível** — a RESTAURAÇÃO de
fato (escrever shows/transações/contatos/metas no banco a partir do arquivo validado) sobre esta base já testada: um passo
de escrita com efeito colateral que exige estratégia de conflito de ids (novo vs. sobrescrever), idealmente com revisão
humana antes de mergear; ou próximas sessões podem evoluir outra feature maior.
**Antes disso, Sessão 359 — EXPORTAÇÃO COMPLETA DA CONTA / BACKUP em `/conta` (`accountExport` + `/conta/dados/export`, D354),
abrindo um eixo NOVO fora do export tabular analítico (que as sessões recentes davam por esgotado):** o app tinha
exports por-lista (`/shows/export`, `/financas/export`, `/contatos/export`, cada um em CSV) e dezenas de CSVs de VISÕES
analíticas, mas nenhuma forma de o músico baixar TODA a sua carteira num único arquivo — um backup / portabilidade dos
próprios dados ("são seus dados", alinhado a F1/ownership e à portabilidade da LGPD). Reunir três CSVs de colunas
diferentes não é backup: perde a relação show↔contato↔transação, o perfil (nome artístico, alíquota) e as metas, e não é
restaurável em princípio. Agora um módulo puro `src/lib/accountExport.ts` — `buildAccountDataExport(input)` monta um
snapshot ÚNICO, versionado e legível por máquina (JSON) com perfil + shows (com `contactIds`, preservando a relação N:N) +
transações (com `showId`) + contatos + metas de faturamento, sob um bloco `meta` (app `Palco`, `schemaVersion`=1 p/
migração futura, `exportedAt` ISO, `counts` por entidade); datas viram ISO 8601, opcionais ausentes viram `null` (JSON
limpo), valores em CENTAVOS crus (backup fiel ao banco). `accountDataExportToJson` serializa indentado e
`accountDataExportFilename(dateKey)` ancora o nome (`palco-meus-dados-YYYY-MM-DD.json`). Camada PURA (sem Prisma, sem
`new Date()`) → testável; a rota `/conta/dados/export` (GET) carrega os quatro conjuntos do usuário (`Promise.all`,
escopados por `userId`), carimba `new Date()` e devolve o JSON como download (`application/json`, `Content-Disposition:
attachment`, `no-store`). Seção "Seus dados" em `/conta` com o botão de download. SEM hashes de senha/tokens no export
(backup de dados do músico, não de credenciais). **+9 testes** (`accountExport.test.ts`: cabeçalho/counts, normalização de
datas/centavos, preservação de `contactIds`, string ISO já pronta, opcionais→null sem `undefined` no JSON, ordem
preservada, conta vazia, round-trip do JSON, nome do arquivo). DoD verde: `npm run build`, `npx tsc --noEmit`,
`npm run lint` (0 warnings), `npm test` (**1943 testes**); smoke → `/login` 200, `/conta` e `/conta/dados/export`
307→/login (auth-gated); **smoke autenticado** (cookie de sessão do usuário demo) → `/conta/dados/export` 200
`application/json`, `Content-Disposition: attachment; filename="palco-meus-dados-2026-07-17.json"`, JSON válido com
`counts` batendo com o seed (3 shows / 5 transações / 3 contatos); `npm audit` inalterado (10 advisories: 4 moderate/5
high/1 critical, ZERO dependência nova), ver D354. **Próximo possível** — importação/restore a partir deste arquivo (o
export é o pré-requisito); incluir os `ShowStatusEvent` (timeline do funil) no snapshot se houver demanda por backup do
histórico; ou próximas sessões podem evoluir outra feature maior.
**Antes disso, Sessão 358 — CSV DOS MOVERS DE PREÇO em `/contatos/retencao` (`retentionPriceMoversToCsv`, D353),
