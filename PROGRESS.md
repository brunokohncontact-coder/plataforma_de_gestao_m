...
do MVP (F1–F5 de `docs/mvp-scope.md`) estão implementadas e navegáveis. **973 testes** verdes após a **exportação CSV do ritmo do mês corrente** em `/financas/ritmo-do-mes/export` (Sessão 167, D160 — a tela `/financas/ritmo-do-mes`/`currentMonthPace` (D158, "estou faturando no ritmo de um mês normal?") ganhou botão "⬇ CSV", fechando a **última lacuna de exportação tabular do acervo**. Novo serializador puro `monthPaceToCsv(pace)` + `MONTH_PACE_CSV_HEADERS` em `src/lib/csv.ts` espelha a tabela "Projeção do mês × mês típico": **não é série temporal** — cada linha é uma **métrica** (Receitas/Despesas/Resultado), não um mês, e por isso **sem linha "Total"** (somar as três métricas não teria sentido; "Resultado" já é o fecho). Colunas Métrica / Já lançado (R$) / Projeção do mês (R$) / Mês típico (R$) / Esperado a esta altura (R$) / Variação proj. × típico (%) — captura todo o número visível da página. Variação por `csvDeltaPct` ("+50%"/"novo"): sem baseline (`insufficient`) emite "novo" onde a página exibe "—" (mesma divergência da D154). Rota reusa a mesma consulta/janela `?meses=` da página + BOM UTF-8; nome `ritmo-do-mes-<YYYY-MM>-<n>m.csv`; botão propaga a janela ativa. **+3 testes** (`describe("monthPaceToCsv")`). Nota de ambiente: o cliente Prisma pré-instalado no container estava dessincronizado do schema (esperava coluna `billingContactId` inexistente), quebrando os testes de server actions; `npx prisma generate` a partir do schema commitado reconciliou e a suíte voltou a 973/973 — sem mudança de código. Smoke test (`next start`) → `/login` 200 e `/financas/ritmo-do-mes` + `/financas/ritmo-do-mes/export` (+ `?meses=12`) 307 (auth-gated). `npm audit` sem novas vulnerabilidades (mesmos advisories Next/postcss da D6)) sobre a Sessão 166. Segue 970 da consolidação da **exportação CSV da agenda de contas a pagar/receber** em `/financas/agenda/export` (PR #180, D157 — a tela "A pagar e receber"/`buildDueAgenda` ganhou botão "⬇ CSV"; serializador puro `dueAgendaToCsv` + `DUE_AGENDA_CSV_HEADERS` em `src/lib/csv.ts`, rótulos de janela extraídos para `DUE_BUCKET_LABELS` em `@/lib/finance` (DRY com a página); **+3 testes**) sobre a Sessão 166. Segue 967 da Sessão 166
...
6b. **Ritmo do mês corrente** (entregue na Sessão 165, `/financas/ritmo-do-mes` + `currentMonthPace` em
   ...
   veredito pela receita (`ahead`/`onPace`/`behind`/`insufficient`, ±10%). **Exportação CSV** entregue na Sessão 167 —
   `monthPaceToCsv` + `MONTH_PACE_CSV_HEADERS` em `src/lib/csv.ts` + `/financas/ritmo-do-mes/export?meses=` (uma linha por
   métrica, sem "Total"; espelha a tabela "Projeção do mês × mês típico"), fechando a **última lacuna de exportação tabular do
   acervo**, ver D160. Próximo possível — uma linha-nudge no
