...
2b. **Funil de propostas — evoluções** (entregue na Sessão 51, `/shows/funil` + `showPipeline`,
   ...
   **Nudge no Painel** entregue na Sessão 247 — `staleProposalsHeadline(report)` + tipo `StaleProposalsHeadline` em
   ...
   ano da taxa de conversão real (card "movers" no espírito da D215/D236, adiado aqui para fechar a 1ª fatia); (b) um nudge no
   Painel se a taxa de conversão da coorte recente cair; (c) quando a amostra de tempo-em-etapa amadurecer, `?ano=`
