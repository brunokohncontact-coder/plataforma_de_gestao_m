...
import { annualSummary, type TxLike } from "@/lib/finance";
import { annualSummaryToCsv } from "@/lib/csv";
...
// Exporta o resumo anual (12 meses + total) em CSV, para o ano da query (?ano=YYYY).
// A camada pura está em `@/lib/csv` (testada).
export async function GET(req: NextRequest) {
  ...
  const csv = annualSummaryToCsv(annualSummary(txs, year));
