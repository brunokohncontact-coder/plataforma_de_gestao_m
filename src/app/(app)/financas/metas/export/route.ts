...
import { monthlyGoalProgress, type TxLike } from "@/lib/finance";
import { monthlyGoalProgressToCsv } from "@/lib/csv";
...
// Exporta a quebra mensal da meta de faturamento (12 meses + total) em CSV, para
// o ano da query (?ano=YYYY). Sem meta definida no ano, a planilha sai com alvos
...
export async function GET(req: NextRequest) {
  ...
  const csv = monthlyGoalProgressToCsv(monthlyGoalProgress(txs, year, goal?.amount ?? 0));

  // BOM UTF-8 para preservar acentuação ao abrir no Excel.
  const body = "﻿" + csv;
  const filename = `metas-mensal-${year}.csv`;
